...
rule all:
    input: expand(join(feat_dir,'task-{task}',subj_sess_dir,'confound-{confound_name}','run-{run}','stats.nii.gz'),task=task,subject=subjects,confound_name=confound_names,run=runs,allow_missing=True)
