if not config['CONTRAST']:
	rule run_cosine_angle:
		...
		output:
			cosine_dictionary = join(confounder_dir,'task-{task}',subj_sess_dir,f'{subj_sess_prefix}_cosine.json'),
			mean_cosine_dictionary = join(confounder_dir,'task-{task}',subj_sess_dir,f'{subj_sess_prefix}_mean_cosine.json')
